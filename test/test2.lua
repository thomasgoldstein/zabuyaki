...
describe("Character Class", function()
    lust.before(function()
        ...
        n = 1
        player1 = HEROES[n].hero:new("PL1-" .. HEROES[n][1].name, getSpriteInstance(HEROES[n].spriteInstance), 0, 0, nil, DUMMY_CONTROL)
        ...
        n = 2
        player2 = HEROES[n].hero:new("PL2-" .. HEROES[n][1].name, getSpriteInstance(HEROES[n].spriteInstance), 0, 0, nil, DUMMY_CONTROL)
        ...
        n = 3
        player3 = HEROES[n].hero:new("PL3-" .. HEROES[n][1].name, getSpriteInstance(HEROES[n].spriteInstance), 0, 0, nil, DUMMY_CONTROL)
    ...
    describe("Chai Class", function()
        describe("Jump Method", function()
            ...
            it('Jumps after running diagonally', function()
                ...
                local xd = absDelta(x, _x)
                local yd = absDelta(y, _y)
                expect(math.floor(xd)).to.equal(117)
        ...
        describe("ChargeDashAttack Method", function()
            it('Attack from the ground', function()
                ...
                local xd = absDelta(x, _x)
                local yd = absDelta(y, _y)
                expect(math.floor(xd)).to.equal(21)
            ...
            it('Attack from just above the ground', function()
                ...
                local xd = absDelta(x, _x)
                local yd = absDelta(y, _y)
                expect(math.floor(xd)).to.equal(149)
                expect(math.floor(y)).to.equal(_y)
            ...
            it('Attack from just above the ground until the 2nd animation', function()
                ...
                local xd = absDelta(x, _x)
                local yd = absDelta(y, _y)
                expect(math.floor(xd)).to.equal(55)
                expect(math.floor(y)).to.equal(_y)
                expect(math.floor(maxZ)).to.equal(13)
